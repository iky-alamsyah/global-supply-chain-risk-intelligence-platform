...
<div class="page-header mb-4">
    <div class="d-flex align-items-center gap-2">
        ...
        <div>
            <h1 class="page-header-title">Edit Port: {{ $port->port_name }}</h1>
            <p class="page-header-sub mb-0">Modify port specifications, coordinate fields, and registry details.</p>
        </div>
    </div>
</div>
        ...
        <form action="{{ route('admin.ports.update', $port) }}" method="POST">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="country_id" class="form-select" required>
                        ...
                        @foreach($countries as $c)
                            <option value="{{ $c->id }}" @selected(old('country_id', $port->country_id) == $c->id)>
                                {{ $c->flag }} {{ $c->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="port_name" value="{{ old('port_name', $port->port_name) }}" class="form-control" required>
                </div>

                {{-- Port Code --}}
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Port Code / LOCODE</label>
                    <input type="text" name="port_code" value="{{ old('port_code', $port->port_code) }}" class="form-control">
                </div>

                {{-- City --}}
                <div class="col-md-4">
                    <label class="form-label fw-semibold">City</label>
                    <input type="text" name="city" value="{{ old('city', $port->city) }}" class="form-control">
                </div>

                {{-- Timezone --}}
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Timezone</label>
                    <input type="text" name="timezone" value="{{ old('timezone', $port->timezone) }}" class="form-control">
                </div>

                {{-- Latitude --}}
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Latitude <span class="text-danger">*</span></label>
                    <input type="number" step="any" name="latitude" value="{{ old('latitude', $port->latitude) }}" class="form-control" required>
                </div>

                {{-- Longitude --}}
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Longitude <span class="text-danger">*</span></label>
                    <input type="number" step="any" name="longitude" value="{{ old('longitude', $port->longitude) }}" class="form-control" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" class="form-select" required>
                        <option value="active" @selected(old('status', $port->status) == 'active')>Active</option>
                        <option value="inactive" @selected(old('status', $port->status) == 'inactive')>Inactive</option>
                    </select>
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" rows="4">{{ old('description', $port->description) }}</textarea>
                </div>
            </div>
